...
end_lat = 35.24863682515971
end_lng = 128.6831870830684
...
# - 10: 최단
# - 30: 최단거리 + 계단제외
search_options = ['0', '4', '10', '30']

for search_option in search_options:
    # Tmap API - URL
    url = f'https://apis.openapi.sk.com/tmap/routes/pedestrian?version=1&format=json&appKey={tmap_api_key}&startX={start_lng}&startY={start_lat}&startName=출발지&endX={end_lng}&endY={end_lat}&endName=목적지&searchOption={search_option}'
...
for idx, coordinates in enumerate(all_coordinates):
    color = ['red', 'green', 'blue', 'purple'][idx % len(search_options)]
    polyline = folium.PolyLine(locations=coordinates, color=color, weight=5).add_to(m)
